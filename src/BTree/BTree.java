...
public class BTree<T extends Comparable<T>> implements ITree<T> {
    ...
    /*
    *
    * Method to get the DOT string format to  get a graph of the tree.
    *
    */
    public String graph() {
    ...
    public String getDOT(Node aux) {
        StringBuilder graph = new StringBuilder();
        graph.append("\"").append(aux.keys[0]).append("\"");
        graph.append("[label=\"");
        ...
        for (int j = 0; j < n; j++) {
            if (!isLeaf) {
                graph.append(getDOT(aux.children[j]));
            }
        }

        if (!isLeaf) {
            graph.append(getDOT(aux.children[n]));
        }

        for (int k = 0; k < n + 1; k++) {
            if (!isLeaf) {
                graph.append("\"").append(aux.keys[0]).append("\":C").append(k).append("->\"").append(aux.children[k].keys[0]).append("\";\n");
            }
        }
